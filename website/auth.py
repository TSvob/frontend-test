...
def password_reset():
    if request.method == 'POST':
        ...
        if user:
            ...
            server.starttls()
            server.login("fake.blockly.reset@gmail.com", "abkxzkwcbsrspjtc")
            server.sendmail("fake.blockly.reset@gmail.com", email, message)
            flash('New password has been sent to your email address.', category='success')
        else:
            flash('Email does not exist.', category='error')
